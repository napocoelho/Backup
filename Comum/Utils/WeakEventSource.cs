...
{
    [Serializable()]
    public class WeakPropertyChangedSource
    {
        ...
        private delegate void OpenEventHandler(object target, object sender, PropertyChangedEventArgs e);
        ...
        private class WeakDelegate
        {
            ...
            public static int GetHashCode(PropertyChangedEventHandler handler)
            {
        ...
        private class DelegateCollection : IEnumerable<WeakDelegate>
        {
            ...
            public void Add(PropertyChangedEventHandler singleHandler)
            {
            ...
            internal void Remove(PropertyChangedEventHandler singleHandler)
            {
            ...
            private void AddToIndex(PropertyChangedEventHandler singleHandler, int index)
            {
                var hashCode = WeakDelegate.GetHashCode(singleHandler);
                if (_index.ContainsKey(hashCode))
    ...
    [Serializable()]
    public class WeakPropertyChangingSource
    {
        private DelegateCollection _handlers;

        public void Raise(object sender, PropertyChangingEventArgs e)
        {
            if (_handlers is null)
                return;
        ...
        public void Subscribe(PropertyChangingEventHandler handler)
        {
            if (handler is null)
                throw new ArgumentNullException(nameof(handler));
            ...
            var singleHandlers = handler
                .GetInvocationList()
                .Cast<PropertyChangingEventHandler>()
                .ToList();
        ...
        public void Unsubscribe(PropertyChangingEventHandler handler)
        {
            if (handler is null)
                throw new ArgumentNullException(nameof(handler));
        ...
        private delegate void OpenEventHandler(object target, object sender, PropertyChangingEventArgs e);
        ...
        private class WeakDelegate
        {
            ...
            private static OpenEventHandler CreateOpenHandler(MethodInfo method)
            {
                var target = Expression.Parameter(typeof(object), "target");
                var sender = Expression.Parameter(typeof(object), "sender");
                var e = Expression.Parameter(typeof(PropertyChangingEventArgs), "e");
            ...
            public bool IsMatch(PropertyChangingEventHandler handler)
            {
                return ReferenceEquals(handler.Target, _weakTarget?.Target)
                       && handler.GetMethodInfo().Equals(_method);
            }

            public static int GetHashCode(PropertyChangingEventHandler handler)
            {
                var hashCode = -335093136;
                hashCode = hashCode * -1521134295 + (handler?.Target?.GetHashCode()).GetValueOrDefault();
                hashCode = hashCode * -1521134295 + (handler?.GetMethodInfo()?.GetHashCode()).GetValueOrDefault();
        ...
        private class DelegateCollection : IEnumerable<WeakDelegate>
        {
            ...
            public void Add(PropertyChangingEventHandler singleHandler)
            {
                _delegates.Add(new WeakDelegate(singleHandler));
                var index = _delegates.Count - 1;
                AddToIndex(singleHandler, index);
            ...
            internal void Remove(PropertyChangingEventHandler singleHandler)
            {
                var hashCode = WeakDelegate.GetHashCode(singleHandler);
            ...
            private void AddToIndex(PropertyChangingEventHandler singleHandler, int index)
            {
                var hashCode = WeakDelegate.GetHashCode(singleHandler);
                if (_index.ContainsKey(hashCode))
                    _index[hashCode].Add(index);
